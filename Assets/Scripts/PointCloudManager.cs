...
public class PointCloudManager : MonoBehaviour {
    ...
    [SerializeField]
    private Vector3 m_ApplyRotation;
	...
	void loadPointCloud(){
        ...
        if (File.Exists(Application.dataPath + dataPath + ".off")) {
            ...
            Debug.Log("Path:" + Application.dataPath + dataPath + ".pts");
        }
    ...
    IEnumerator LoadPTS(string dPath) {
        ...
        Debug.Log(buffer[0]);
        numPoints = int.Parse(buffer[0]) / 1;
        ...
        for (int i = 0; i < numPoints; i = i + 1)
        {
            ...
            if (buffer.Length >= 5)
                ...
                colors[i] = (new Color(float.Parse(buffer[4]), float.Parse(buffer[5]), float.Parse(buffer[6]))) * float.Parse(buffer[3]);
            else
                colors[i] = Color.cyan;
	...
	void InstantiateMesh(int meshInd, int nPoints){
        ...
        pointGroup.transform.rotation = Quaternion.Euler(m_ApplyRotation);

        /*Quaternion.e

        pointGroup.transform.rotation.eulerAngles(m_ApplyRotation);*/
